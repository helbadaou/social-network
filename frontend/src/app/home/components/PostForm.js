...
export default function PostForm({ content, setContent, image, setImage, privacy, setPrivacy, handleSubmit, creating }) {
  return (
    <form onSubmit={handleSubmit} className="bg-gray-900 p-4 rounded-xl shadow mb-6 flex flex-col gap-4 border border-gray-700">
      ...
      />
      <select value={privacy} onChange={(e) => setPrivacy(e.target.value)} className="bg-gray-800 border border-gray-700 p-2 rounded text-sm text-white">
        <option value="public">Public</option>
        <option value="followers">Abonnés</option>
        <option value="private">Privé</option>
      </select>
      <button type="submit" className="bg-blue-600 text-white p-2 rounded hover:bg-blue-700" disabled={creating}>
